  ...
  public function payment_crypto_verify()
    ...
    $installment_fees = array(
        ...
        wc_get_template(
			...
			array(
				...
				'allow_cartao_payment' => $this->allow_cartao,
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			...
			'allow_cartao' => array(
					  'title'		=> __('Cartão de Crédito', 'tuna-payment'),
					  'label'		=> __('Habilitar Cartão de crédito', 'tuna-payment'),
					  'type'		=> 'checkbox',
					  'default'	=> 'yes',
				  ),
      'allow_boleto' => array(
      ...
      'installment_fee1' => array(
        ...
        'description' => __('Valores em unidades percentuais (%)', 'tuna-payment'),
        'type'		=> 'text',
				'required'  => true,
	...
	private function get_feed_id($order,$name)
    {
        $fees = $order->get_fees();
	...
	public function process_payment($order_id)
	{
		...
		if (sanitize_text_field($_POST["tuna_is_boleto_payment"]) == "true") {
            ...
            $order_subtotal = floatval($customer_order->get_total());
            ...
            $installment_amount = get_installment_amount($order_subtotal, $fee);
            $installment_amount = number_format($installment_amount, 2, '.', '');			 
			
			wc_delete_order_item($this->get_feed_id($customer_order,$nameJuros));			
			wc_delete_order_item($this->get_feed_id($customer_order,$nameDiscount));
			$customer_order->calculate_taxes();
            ...
            if ($installment_amount > 0) {
              ...
              // Fee object body
              $order_fee = new WC_Order_Item_Fee();
              $order_fee->set_name($nameDiscount);
              $order_fee->set_amount($installment_amount);
              $order_fee->set_total($installment_amount);
