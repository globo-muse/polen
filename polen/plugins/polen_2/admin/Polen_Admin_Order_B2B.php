...
class Polen_Admin_Order_B2B
{
    ...
    public function new_order_handler($order_id)
    {
        ...
        if( is_admin() && $screen->base == 'post' && $screen->post_type == 'shop_order' && current_user_can('manage_options') ) {
            ...
            # ADD
            wc_update_order_item_meta($item_order->get_id(), 'form_of_payment', $form_of_payment, false);
            wc_update_order_item_meta($item_order->get_id(), 'value_payment_talent', $value_payment_talent, false);
            wc_update_order_item_meta($item_order->get_id(), 'payday', $payday, false);
